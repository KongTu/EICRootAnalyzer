...
void run_DMPJet3( int nEvents, bool doBoost, TString inputFilename ) {
   ...
   for(int i(0); i < nEvents; ++i ) {
      ...
      double single_nucleon_energy = sqrt(pztarg*pztarg + MASS_NEUTRON);

      // double gamma_ion = total_energy/MASS_DEUTERON;
      // double bz = pz_total/(gamma_ion*MASS_DEUTERON);
      double gamma_ion = single_nucleon_energy/MASS_NEUTRON;
      double bz = pztarg/(gamma_ion*MASS_NEUTRON);
