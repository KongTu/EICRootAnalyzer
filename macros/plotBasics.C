...
void plotBasics(){
	...
	for(int i = 0; i < 16; i++){
		...
		energyVsPtf_2Dcorr[i]->GetXaxis()->SetRangeUser(-1,2);
		energyVsPtf_2Dcorr[i]->Draw("colz");

		c1[i]->cd(9);
		gPad->SetLogz();
		energyVsProcess_2Dcorr[i]->SetTitle("process vs E_{in}-E_{out}");
		energyVsProcess_2Dcorr[i]->GetXaxis()->SetRangeUser(-1,2);
